...
file_path = 'C:/Users/shofmann/Lab Work/Analysis Data/4_11_2024/' # folder with H5 data files
run_spe_solicited = RunInfo([file_path+'Run_1712873075.hdf5'], do_filter = True, is_solicit = True, upper_limit = 1, baseline_correct = True)
# loop quickly loads in files
# separate files for each bias voltage -> specifyAcquisition = False
files = ['Run_1712867521.hdf5','Run_1712870910.hdf5', 'Run_1712872359.hdf5', 'Run_1712864542.hdf5', 'Run_1712869928.hdf5', 'Run_1712868713.hdf5', 'Run_1712871709.hdf5', 'Run_1712865181.hdf5'] #,'Run_1712866338.hdf5'
proms = [0.005,0.0057,0.0057,0.0057, 0.0057,0.0057,0.0057 ,0.006, .008, .0057,.0057]
upperlim = [.3, .3, .3, .3, .3, .3, .3, .3, .3 ,.3, .3]
runs = [] 
...
#%%
runs[5] = RunInfo([file_path +files[5]], specifyAcquisition = False, do_filter = True, upper_limit = .3 , baseline_correct = True, prominence = 0.0052 , poly_correct = True, plot_waveforms = False, is_led=True)
#%%
...
for run in runs:
    ...
    range_low =  centers[peaks[0]]- 0.18*fitrange
    range_high = centers[peaks[3]]+ 0.24*fitrange
...
def get_subtract_hist_mean(bias, data1, data2, numbins = 2000, plot = False):
    if plot:
        ...
        plt.axvline(x = (np.mean(data1) + np.std(data1)/np.sqrt(len(data1))), color = 'blue',linestyle = ':')
        plt.axvline(x = (np.mean(data1) - np.std(data1)/np.sqrt(len(data1))), color = 'blue',linestyle = ':')
        print('LED on hist: ' + str(np.mean(data1)) + ", " + str(np.std(data1)/np.sqrt(len(data1))))
        print('LED off hist: ' + str(np.mean(data2))+ ", " + str(np.std(data2)/np.sqrt(len(data2))))
        plt.axvline(x = np.mean(data2), color = 'orange')
        plt.axvline(x = (np.mean(data2) - np.std(data2)/np.sqrt(len(data2))), color = 'orange',linestyle = ':')
        plt.axvline(x = (np.mean(data2) + np.std(data2)/np.sqrt(len(data2))), color = 'orange',linestyle = ':')
    ...
    if plot:
        ...
        plt.axvline(x = (mean_value + mean_err), color = 'green', linestyle = ':')
        plt.axvline(x = (mean_value - mean_err), color = 'green', linestyle = ':')
